...
import { confirm, save, open, message } from './api/dialog.js';
import { invoke } from './api/tauri.js';
import { readTextFile } from './api/fs.js';
import { resolveResource } from './api/path.js';
...
async function saveGame() {
  ...
  if (gameTitle == '') {
      return await message('You must provide a title before saving.', { title: 'ifSpace', type: 'error' });
  }

  let gamePath = await save({
    defaultPath: `${gameTitle}.json`,
    title: "Save game"
  });
  ...
  if (gamePath) {
      ...
      invoke('save_game', {'savePath': gamePath, 'gameFile': JSON.stringify(gameFile)});
  } else {
    return;
  }
}
...
async function loadGame() {
  ...
  if (file) {
    let gameData = await(invoke('load_game', {'loadPath': file}));
    let loadData = JSON.parse(gameData);
...
async function exportGame() {
  ...
  if (gameTitle == '') {
      return await message('You must provide a title before exporting.', { title: 'ifSpace', type: 'error' });
  }
  ...
  let resourcePath = await resolveResource('export.txt');
  let exportFile = await readTextFile(resourcePath);
  exportFile = exportFile.replace("MASTERIFSPACEGAMEFILE", JSON.stringify(gameFile));

  let filename = `${gameTitle}.html`;

  let exportPath = await save({
    defaultPath: filename,
    title: "Save game"
  });

  invoke('download_export', {'downloadPath': exportPath, 'exportFile': exportFile});
}
...
async function deleteNode () {
    if (cNode.location.toString() !== "0,0,0") {
        if (await confirm("Are you sure you want to delete the current node?", {title: 'ifSpace', type: 'warning'})) {
