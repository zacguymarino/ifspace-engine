    ...
    for (let input of inputs) {
        ...
        if ($(input).is(':checked')) {
            if (!$(`#${inputId}_Edit`).length) {
                ...
                let html= newDivStart + direction + divLocLabel + divLoc + divAltLabel + divAlt + excludeDefaults +
                    requirements + newDivEnd;
...
function addContainer () {
    ...
    let containerId;
    if (length >= 1) {
        let lastId = +$('#containerList').children().last().attr('id').split("_")[1];
    ...
    let html = newDivStart + rmvButton + nameLabel + name + capLabel + cap + completeLabel + complete + illegalLabel + illegal + requirements + newDivEnd;
...
function addEvo(listId) {
    ...
    let html = newDivStart + evoHeading + requirements + evoDesLabel + evoDes + rmvButton + newDivEnd;
...
function getRequirements(baseId) {
    ...
    let html = reqLabel + reqItems + reqContainers + reqLocal + reqGlobal + reqVisits + reqEvos;
    return html;
}
...
export { directions, addItem, addContainer, addAction, showHide, addEvo, getRequirements, removeEvo, removeObject, loadDomFromNode };