...
///GAME PLAYING///
//////////////////
var game = adventure.gameContent;
...
var monitors = adventure.monitors;
var customDeposits = adventure.customDeposits;
var customWithdrawals = adventure.customWithdrawals;
var customTakes = adventure.customTakes;
var customDrops = adventure.customDrops;
var customIgnorables = adventure.customIgnorables;
var customLooks = adventure.customLooks;
var customExamines = adventure.customExamines;
var globalWin = adventure.globalWin;
var globalLose = adventure.globalLose;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("restartGameSim").addEventListener("click", function() {
        if (confirm("Restart game?  Any progress will be lost.")) {
            restartGame(null);
        }
    })

    document.getElementById("saveLoadButton").addEventListener("click", function() {
        document.getElementById("saveLoadPopup").style.visibility = "visible";
    })

    document.getElementById("popupClose").addEventListener("click", function() {
        document.getElementById("saveLoadPopup").style.visibility = "hidden";
    })

    document.getElementById("saveProgress").addEventListener("click", function() {
        save["html"] = document.getElementById("outputSim").innerHTML;
        let filename = `${gameTitle}_${new Date().toLocaleString()}.json`;
...
function updateInputEvents() {
    function updateClassic(event) {
        let classicInput = document.getElementById("classicStyleInput");
        if (event.which == 13) {
            try {
                parseAction(classicInput.value);
                classicInput.removeEventListener("keydown", updateClassic);
            } catch (error) {
                console.log(error);
            }
            classicInput.value = "";
        ...
        if (event.which == 38) {
            classicInput.value = previousInput;
        }
    }

    function updateModernAction(event) {
        let eventValue = event.currentTarget.value;
        try {
            parseAction(eventValue);
            event.currentTarget.removeEventListener("click", updateModernAction);
        } catch {
            console.log(error);
        }
        event.preventDefault();
    ...
    function updateModernDirections(event) {
        let eventValue = event.currentTarget.value;
        try {
            parseAction(eventValue);
            event.currentTarget.removeEventListener("click", updateModernDirections);
        } catch {
            console.log(error);
        }
        event.preventDefault();
    ...
    function updateGamebook(event) {
        let eventValue = event.currentTarget.value;
        try{
            parseAction(eventValue);
            event.currentTarget.removeEventListener("click", updateGamebook);
        } catch {
            console.log(error);
        }
        event.preventDefault();
    ...
    switch (gameStyle) {
        case "classic":
            let classicInput = document.getElementById("classicStyleInput");
            classicInput.addEventListener("keydown", updateClassic);
            break;

        case "modern":
            let modernActionButtons = document.getElementsByClassName("modernActionButton");
            for (let i = 0; i < modernActionButtons.length; i++) {
                modernActionButtons[i].addEventListener("click", updateModernAction);
            }
            let modernDirectionButtons = document.getElementsByClassName("modernDirectionButton");
            for (let i = 0; i < modernDirectionButtons.length; i++) {
                modernDirectionButtons[i].addEventListener("click", updateModernDirections);
            }
            break;
        
        case "gamebook":
            let gamebookButtons = document.getElementsByClassName("gamebookDirectionButton");
            for (let i = 0; i < gamebookButtons.length; i++) {
                gamebookButtons[i].addEventListener("click", updateGamebook);
            }
            break;
...
function addSimInput() {
  let inputSimDiv = document.createElement("div");
  inputSimDiv.setAttribute("id", "inputSim");
  switch(gameStyle) {
    case "classic":
        let classicInput = document.createElement("input");
        classicInput.setAttribute("id", "classicStyleInput");
        classicInput.setAttribute("type", "text");
        
        inputSimDiv.appendChild(classicInput);
        break;
    case "modern":
        let modernDirections = document.createElement("div");
        modernDirections.setAttribute("id", "modernStyleDirections");
        modernDirections.classList.add("modernInput");
        let modernActions = document.createElement("modernInput");
        modernActions.setAttribute("id", "modernStyleActions");
        modernActions.classList.add("modernInput");
        inputSimDiv.appendChild(modernDirections);
        inputSimDiv.appendChild(modernActions);
        break;
    case "gamebook":
        let gamebookDirections = document.createElement("div");
        gamebookDirections.setAttribute("id", "gamebookStyleDirections");
        gamebookDirections.classList.add("gamebookInput");
        inputSimDiv.appendChild(gamebookDirections);
        break;
    default:
        let errorP = document.createElement("p");
        errorP.innerText = "You must first load a saved game.";
        inputSimDiv.appendChild(errorP);
        break;
  }
  if (document.getElementById("inputSim") != null) {
    document.getElementById("inputSim").remove();
  }
  document.getElementById("gameContainer").appendChild(inputSimDiv);
}

///////////////
///GAME INIT///
///////////////
async function gameInit(load) {
    ...
    if (load != null) {
        ...
        previousNode = save["previousNode"];
        document.getElementById("outputSim").innerHTML = save["html"];
    } else {
        save = {
            "html": '',
        ...
        if (IFID) {
            gameSimIFID = document.createElement("h6");
        ...
        let outputSimElement = document.getElementById("outputSim");
        outputSimElement.innerHTML = "";
...
function updateModernDirections(directionList) {
    let modernStyleDirections = document.getElementById("modernStyleDirections");
    modernStyleDirections.innerHTML = "";
    for (let i = 0; i < directionList.length; i++) {
        let buttonValue = `${directionList[i].alternatives[0]}`;
        let button = document.createElement("button");
        button.classList.add("modernDirectionButton");
        button.classList.add("inputButton");
        button.setAttribute("value", buttonValue);
        button.innerText = directionList[i].alternatives[0];
        modernStyleDirections.appendChild(button);
    }
}

function updateModernActions(actionList) {
    let modernStyleActions = document.getElementById("modernStyleActions");
    modernStyleActions.innerHTML = "";
    for (let i = 0; i < actionList.length; i++) {
        let button = document.createElement("button");
        button.classList.add("modernActionButton");
        button.classList.add("inputButton");
        button.setAttribute("value", actionList[i]);
        button.innerText = actionList[i];
        modernStyleActions.appendChild(button);
    }
}

function updateGamebookDirections(directionList) {
    let gamebookStyleDirections = document.getElementById("gamebookStyleDirections");
    gamebookStyleDirections.innerHTML = "";
    for (let i = 0; i < directionList.length; i++) {
        let buttonValue = `${directionList[i].alternatives[0]}`;
        let button = document.createElement("button");
        button.classList.add("gamebookDirectionButton");
        button.classList.add("inputButton");
        button.setAttribute("value", buttonValue);
        button.innerText = directionList[i].alternatives[0];
        gamebookStyleDirections.appendChild(button);
    }
}
...
function handlePrimaryUseVerbs(actionVerbs, userActionWords, userItems) {
...
function parseAction(input) {
    if (playing) {
        //Reset scrolls
        let scrollTos = document.getElementsByClassName("scrollTo");
        while (scrollTos.length) {
            scrollTos[0].classList.remove("scrollTo");
        }
        ...
        for (let i = 0; i < globalActions.length; i++) {
            ...
            let foundItemPrimaryUses = handlePrimaryUseVerbs(actionVerbs, userActionWords, userItems);
        ...
        if (sentMessage != true) {
            ...
            if (cContainerItemExamines.includes(action)) {
                ...
                if (examineItem(actionItem, inspectableItems)) {
                    sentMessage = true;
                };
            }
