...
function addVisit(location) {
  save.nodes[location].visits = (
    Number(save.nodes[location].visits) + 1
  ).toString();
}
...
function displayMessage(message, input) {
  ...
  if (input) {
    $("#outputSim").append(
      `<p class='scrollTo'><b>&nbsp;&nbsp;>&nbsp;${message}</b></p>`
    );
  } else {
...
function getActions(location) {
  ...
  for (let i = 0; i < game[currentNode].actions.actions.length; i++) {
    let variants =
      game[currentNode].actions.actions[i].actions.split(/\s*,\s*/);
  ...
  for (let i = 0; i < save.nodes[location].containers.length; i++) {
    if (save.nodes[location].containers[i].items.length > 0) {
      let containerVariants =
        save.nodes[location].containers[i].name.split(/\s*,\s*/);
      let itemArray = [];
      for (
        let j = 0;
        j < save.nodes[location].containers[i].items.length;
        j++
      ) {
        let itemVariants =
          save.nodes[location].containers[i].items[j].name.split(/\s*,\s*/);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    for (let j = 0; j < variants.length; j++) {
      for (let k = 0; k < itemInspectCommands.length; k++) {
        let newAction = `${itemInspectCommands[k]} ${variants[
          j
        ].toUpperCase()}`;
    ...
    if (exclude !== "true") {
      switch (thisDirection) {
        ...
        case "NW":
          ...
          break;
        case "Up":
          directionObject["alternatives"] = ["GO UP", "TRAVEL UP"];
          break;
        case "Down":
          directionObject["alternatives"] = ["GO DOWN", "TRAVEL DOWN"];
          break;
      }
    }
...
function checkRequirements(reqs) {
  ...
  let reqContainers = !reqs["reqContainers"]
    ? []
    : reqs["reqContainers"].split(/\s*,\s*/);
  let reqLocal = !reqs["reqLocal"] ? [] : reqs["reqLocal"].split(/\s*,\s*/);
  let reqGlobal = !reqs["reqGlobal"] ? [] : reqs["reqGlobal"].split(/\s*,\s*/);
  let locVisits = !reqs["locVisits"]
    ? []
    : reqs["locVisits"].split(/\]\s*,\s*/);
  ...
  for (let i = 0; i < reqContainers.length; i++) {
    for (let j = 0; j < save.nodes[currentNode].containers.length; j++) {
      let variants =
        save.nodes[currentNode].containers[j].name.split(/\s*,\s*/);
          ...
          if (reqContainers[i] === containerName) {
            ...
            if (!isNaN(parseInt(completeReqs))) {
              ...
              for (let k = 0; k < completeReqs.length; k++) {
                if (
                  !save.containers[reqContainer[i]].includes(completeReqs[k])
                ) {
                  return false;
                }
              }
    ...
    if (quant > 0) {
      if (!save.nodes.hasOwnProperty(loc)) {
        ...
        if (Number(save.nodes[loc].visits) < quant) {
          console.log(
            "Have not been to location enough times. LocVisits req failed."
          );
          return false;
        }
      }
...
function checkWin() {
  ...
  if (checkRequirements(reqs) && win.description.length > 0) {
    ...
    if (max != 0) {
      ...
      displayMessage(
        "Score: " + points.toString() + "/" + max.toString(),
        false
      );
    }
  }
...
function displayItems() {
  let validItems = [];
  if (
    save.nodes[currentNode].items.length > 0 &&
    ...
    for (let i = 0; i < save.nodes[currentNode].items.length; i++) {
      ...
      if (checkRequirements(reqs)) {
        let mainName =
          save.nodes[currentNode].items[i].name.split(/\s*,\s*/)[0];
...
function parseAction(input) {
  ...
  for (let h = 0; h < cNodeActions.length; h++) {
    if (cNodeActions[h].toUpperCase() == action) {
      for (let i = 0; i < game[currentNode].actions.actions.length; i++) {
        if (
          game[currentNode].actions.actions[i].actions
            .toUpperCase()
            .includes(action)
        ) {
          let actionObject = JSON.parse(
            JSON.stringify(game[currentNode].actions.actions[i])
          );
          let reqs = {
          ...
          if (checkRequirements(reqs)) {
            let mainAction = actionObject.actions
              .split(/\s*,\s*/)[0]
              .toUpperCase();
            let maxTimes;
        ...
        for (let k = 0; k < cContainerWithdrawals.items[j].length; k++) {
          for (let l = 0; l < cContainerWithdrawals.items[j][k].length; l++) {
            let regexp2 = new RegExp(
              `\s*${cContainerWithdrawals.items[j][k][l].toUpperCase()}\s*`
            );
            if (action.match(regexp2)) {
              ...
              for (let m = 0; m < containerVariants.length; m++) {
                let regexp3 = new RegExp(
                  `\s*${containerVariants[m].toUpperCase()}\s*`
                );
                if (action.match(regexp3)) {
                  ...
                  for (
                    let q = 0;
                    q < game[currentNode].containers.length;
                    q++
                  ) {
                    if (
                      game[currentNode].containers[q].name
                        .toUpperCase()
                        .match(regexp3)
                    ) {
                      reqs = {
                        ...
                        reqContainers:
                          game[currentNode].containers[q].reqContainers,
                  ...
                  if (checkRequirements(reqs)) {
                    ...
                    for (
                      let n = 0;
                      n < save.nodes[currentNode].containers.length;
                      n++
                    ) {
                      if (
                        save.nodes[currentNode].containers[n].name.includes(
                          containerVariants[m]
                        )
                      ) {
                        for (
                          let p = 0;
                          p <
                          save.nodes[currentNode].containers[n].items.length;
                          p++
                        ) {
                          if (
                            save.nodes[currentNode].containers[n].items[
                              p
                            ].name.includes(
                              cContainerWithdrawals.items[j][k][l]
                            )
                          ) {
                            let item = JSON.parse(
                              JSON.stringify(
                                save.nodes[currentNode].containers[n].items[p]
                              )
                            );
                            save.nodes[currentNode].containers[n].items.splice(
                              p,
                              1
                            );
    ...
    if (action.match(regexp1)) {
      ...
      for (let j = 0; j < cContainerDeposits.items.length; j++) {
        for (let k = 0; k < cContainerDeposits.items[j].length; k++) {
          let regexp2 = new RegExp(
            `\s*${cContainerDeposits.items[j][k].toUpperCase()}\s*`
          );
          if (action.match(regexp2)) {
            ...
            for (let l = 0; l < cContainerDeposits.containers.length; l++) {
              for (
                let m = 0;
                m < cContainerDeposits.containers[l].length;
                m++
              ) {
                let regexp3 = new RegExp(
                  `\s*${cContainerDeposits.containers[l][m].toUpperCase()}\s*`
                );
                if (action.match(regexp3)) {
                  ...
                  if (checkRequirements(reqs)) {
                    ...
                    for (let n = 0; n < save.items.length; n++) {
                      let variants = save.items[n].name
                        .toUpperCase()
                        .split(/\s*,\s*/);
                      if (
                        variants.includes(
                          cContainerDeposits.items[j][k].toUpperCase()
                        )
                      ) {
                        if (
                          !thisContainer.illegal
                            .toUpperCase()
                            .includes(variants[0])
                        ) {
                          ...
                          for (
                            let p = 0;
                            p < save.nodes[currentNode].containers.length;
                            p++
                          ) {
                            let saveVariants =
                              save.nodes[currentNode].containers[p].name.split(
                                /\s*,\s*/
                              );
                            if (
                              saveVariants.includes(
                                cContainerDeposits.containers[l][m]
                              )
                            ) {
                              save.nodes[currentNode].containers[p].items.push(
                                item
                              );
                              displayMessage("Done.", false);
                              sentMessage = true;
  ...
  for (let i = 0; i < cNodeDirections.length; i++) {
    if (cNodeDirections[i].alternatives.includes(action)) {
      if (checkRequirements(cNodeDirections[i].requirements)) {
        ...
        displayMessage(
          "Something is preventing you from going this way.",
          false
        );
        sentMessage = true;
      }
    ...
    for (let i = 0; i < save.nodes[currentNode].items.length; i++) {
      ...
      for (let j = 0; j < variants.length; j++) {
        if (variants[j].toUpperCase() === actionItem) {
          let discoveredItem = JSON.parse(
            JSON.stringify(save.nodes[currentNode].items[i])
          );
