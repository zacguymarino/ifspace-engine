...
function nodeMap () {
    ...
    const reset = (e) => {
        if (Math.sqrt(Math.pow(dragTotalX, 2) + Math.pow(dragTotalY, 2)) <= Math.sqrt(2) && possibleClick) {
            getClickCoords(e);
        }
...
function getClickCoords (event) {
    ...
    let y = ((event.clientY - universe.offsetTop + $(document).scrollTop()) - yTransform)/step;
    let z = $('#zIndex').val();
    let coordsTitle;
    x = Math.floor(x);
    y = -1*Math.floor(y);
